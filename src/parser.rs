...
pub fn parse_xml<'a>(xml_content: &'a str, file_path: &'a str) -> Result<Rc<RefCell<Node<'a>>>, Box<dyn Error>> {
    ...
    for token_result in tokenizer {
        match token_result {
            ...
            Err(e) => {
                ...
                print_diag_error(Some(file_path), xml_content, DiagInfo {
                    message: e.to_string(),
                    label: Some("here"),
                    note: None,
